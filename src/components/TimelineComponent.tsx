...
import {
  ...
  initConfig as initBranchConfig,
} from "../pages/flowPages/Branch/Config";
import {
  generateCycleConfig,
  initConfig as initCycleConfig,
} from "../pages/flowPages/Cycle/Config";
import {
  generateCheckConfig,
  initConfig as initCheckConfig,
} from "../pages/flowPages/Check/Config";
...
import { DialogProxy } from "../pages/flowPages/SelectTemplate";
import { handleUpdateComponent } from "../pages/selections/ComponentEditor";
...
const getNewName = (model: PlayModel, component: ComponentType) => {
  const componentTable = {
    Branch: "Branching Module ",
    Cycle: "Loop Module ",
    Check: "Comparing Module ",
  };
  let maxNum = 0;
  for (const i in model.timeNodes) {
    const node = model.timeNodes[i];
    if (node.data.component?.startsWith(componentTable[component])) {
      let num = parseInt(/\d+/.exec(node.data.component)?.[0] ?? "0");
      maxNum = Math.max(maxNum, num);
    }
  }
  return componentTable[component] + Math.floor(maxNum + 1);
};
...
export const TimelineComponent = (props: {
  ...
  const [popoverAnchor2, setPopoverAnchor2] =
    useState<HTMLButtonElement | null>(null);
  const [showComponentDialog, setShowComponentDialog] = useState<{
    type: string;
    name: string;
  } | null>(null);
  ...
  const setGlobalTime = (time: [string, number]) => {
    ...
    chgModel((model) => {
      const componentName = model.timeNodes[time[0]]?.data.component ?? null;
      const componentType = model.timeNodes[time[0]]?.data.type ?? null;
      model.templateData.currentConfig =
  ...
  const handleAddToolClick = (component: ComponentType, isReplace: boolean) => {
    ...
    const name = getNewName(model, component);
    chgModel((draft) => {
      let parent = timeNodes.objs[globalTime[0]].parent;
      const config = componentFunc[component][0]();
      config.prefix = name;
      const newStart = name + componentFunc[component][2];

      let nthChild = 0;
      for (const i of draft.timeArrs[parent ?? "_null"]) {
        if (i === globalTime[0]) {
          break;
        }
        nthChild += 1;
      ...
      if (depth % 2) {
        // 并行时需要加一个Group
        if (isReplace) {
          draft.timeArrs[globalTime[0]] = [newStart];
          parent = globalTime[0];
          nthChild = 0;
        } else {
          draft.timeArrs[parent ?? "_null"].push(newStart + "_Group");
          draft.timeArrs[newStart + "_Group"] = [];
          draft.timeNodes[newStart + "_Group"] = {
            id: newStart + "_Group",
            parent: parent,
            width: 144,
            data: parent ? timeNodes.objs[parent].data : {},
            ports: [],
          };
          parent = newStart + "_Group";
        }
      }

      if (isReplace) {
        for (const i in draft.timeNodes) {
          const node = draft.timeNodes[i];
          for (const j of node.ports) {
            if (j.target === globalTime[0]) {
              j.target = newStart;
            }
          }
        }

        for (const i of draft.events) {
          if (i.args.target === globalTime[0]) {
            i.args.target = newStart;
          }
        }

        if (draft.objTab[draft.root].rootLayout === globalTime[0]) {
          draft.objTab[draft.root].rootLayout = newStart;
        }

        draft.timeArrs[parent ?? "_null"].splice(nthChild, 1);

        componentFunc[component][1](draft, config, false, parent, nthChild);
      } else {
        componentFunc[component][1](draft, config, false, parent, -1);
      }
    });
    setPopoverAnchor(null);
    setPopoverAnchor2(null);
    setShowComponentDialog({ type: component, name: name });
  };
  ...
  const deleteScene = (
    ...
    let newGlobalTime: typeof globalTime = [...globalTime];

    // 使用draft和modelDraft执行删除
    let toRemove: string | null = name;
  ...
  const handleDeleteToolClick = (toDel?: string) => {
    const delComponent = toDel ?? globalTime[0];
    if (!delComponent) {
      return;
    }
    const componentToDelete = timeNodes.objs[delComponent].data.component;
    // 如果不是一个组件，删除单个场景
    if (!componentToDelete) {
      chgTimeNodes((draft, modelDraft) => {
        const newGlobalTime = deleteScene(delComponent, draft, modelDraft!);
        if (!toDel) {
    ...
    let toDelete = delComponent;
    // 找到该组件的最上层
    while (true) {
      const parent = timeNodes.objs[toDelete].parent;
      if (
        !parent ||
        timeNodes.objs[parent].data.component !== componentToDelete
      ) {
        break;
      }
      toDelete = parent;
    }
    let allComponentData: string[] = [];
    for (const i of timeNodes.arrs[
      timeNodes.objs[toDelete].parent ?? "_null"
    ]) {
      if (timeNodes.objs[i].data.component === componentToDelete) {
        allComponentData.push(i);
      }
    }
    chgTimeNodes((draft, modelDraft) => {
      for (const i of allComponentData) {
        if (i === toDelete) continue;
        deleteScene(i, draft, modelDraft!);
      }
      let newGlobalTime = deleteScene(toDelete, draft, modelDraft!);
      if (!toDel) {
        setGlobalTime(newGlobalTime);
      }
      console.log(newGlobalTime);
    });
  };
  ...
  const handleCancelDialog = () => {
    if (showComponentDialog?.name) {
      let name = showComponentDialog.name;
      for (const i of model.timeArrs[currentLayer ?? "_null"]) {
        if (model.timeNodes[i].data.component === showComponentDialog.name) {
          name = i;
          break;
        }
      }
      handleDeleteToolClick(name);
    }
    setShowComponentDialog(null);
  };
  ...
  return (
    <Box onKeyDown={(e) => handleKeyPress(e)} tabIndex={0} sx={{position: "relative", width: "100%", height: "100%"}}>
      ...
      >
        ...
        >
          <MenuItem onClick={() => handleAddToolClick("Branch", false)}>
            Branching Module
          </MenuItem>
          <MenuItem onClick={() => handleAddToolClick("Cycle", false)}>
            Loop Module
          </MenuItem>
          <MenuItem onClick={() => handleAddToolClick("Check", false)}>
            Comparison Module
          </MenuItem>
        </Menu>
        <Dialog
          open={Boolean(showComponentDialog)}
        ...
        >
          ...
          >
            <DialogProxy
              data={showComponentDialog!}
              onButtonClick={handleCancelDialog}
              onSuccessClick={() => {
                setShowComponentDialog(null);
                handleUpdateComponent(
                  showComponentDialog?.name,
                  showComponentDialog?.type,
                  chgModel
                );
              }}
            />
