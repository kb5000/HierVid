...
export const BranchDialog = (props: {data: {name?: string}, onSuccessClick: () => void, onButtonClick: () => void}) => {
  const theme = useTheme();
  const { enqueueSnackbar } = useSnackbar();
  const [changed, setChanged] = useState(false);
  const [model, chgModel] = useContext(ModelEditContext)!;
  const [unnamedConfig, chgUnnamedConfig] = useImmer<BranchConfig>(
  ...
  const chgNamedConfig = (config: (draft: BranchConfig) => void) => {
    chgModel(draft => {
      config(draft.templateData.componentConfig[props.data.name!] as BranchConfig)
    })
  }
  const config = props.data.name ? namedConfig as BranchConfig : unnamedConfig
    ...
    <>
      <Box p={2} sx={{ overflow: "hidden" }}>
        ...
        <Stack direction="row" mt={1} px={1}>
          <Box flexGrow={1} mr={2}>
            <Typography variant="h4">{props.data.name ? "Branching Module" : "Branching Story Template"}</Typography>
            <Typography
              sx={{
                mt: 1,
                wordWrap: "break-word",
          ...
          >
            <Box
              component="img"
              src={process.env.PUBLIC_URL + "/img/模版1-剧情分支_640.jpg"}
              alt={"模板1"}
              sx={{
                position: "relative",
                left: "-8px",
...
export const BranchComponentSetting = (props: {name: string}) => {
  const { enqueueSnackbar } = useSnackbar();
  const [model, chgModel] = useContext(ModelEditContext)!;
  const config = model.templateData.componentConfig[props.name] as BranchConfig
  const chgConfig = (config: (draft: BranchConfig) => void) => {
    chgModel(draft => {
