...
export const CheckEdit = (props: any) => {
  ...
  const handleNext = () => {
    if (page === 1) {
      chgModel(draft => {
        clearModel(draft)
        generateCheckConfig(draft, config, true, null)
      });
      props.handleNext();
    } else {
      setPage((p: number) => p + 1);
      ...
      >
        ...
        <Stack sx={{ alignItems: "center", flexGrow: 1 }}>
          <Typography fontWeight="500" mt={1}>
            {
              [
                "Step 1 / 2：Upload the opening",
                "Step 2 / 2：Upload video of each branch",
              ][page]
            }
          </Typography>
          <Box
                ...
                <If v-if={page === 0}>
                  {Object.entries(config.videoText).map((x, idx) => {
                    if (x[0].startsWith("branch_")) {
                      ...
                      return (
                        <ResizableBox
                          key={JSON.stringify([x[0], text, boxSize])}
                          state={{
                          ...
                          bound={boxSize}
                          onStateChanged={(y) => handleStateChanged(idx, y)}
                        >
                          <Input
                            sx={{
                              position: "relative",
